---
title: "Using the tidytreatment package with stochtree"
author: "Joshua J Bon"
date: "`r Sys.Date()`"
bibliography: vignette.bib
output: rmarkdown::html_vignette
vignette: >
  %\VignetteIndexEntry{Using the tidytreatment package with stochtree}
...
This vignette demonstrates an example workflow for heterogeneous treatment effect models using the `stochtree` package for fitting Bayesian Additive Regression Trees and `tidytreatment` for investigating the output of such models. The `tidytreatment` package can also be used with other BART packages, see other vignettes for details. 
...
Run the model to be used to assess treatment effects. Here we will use the `stochtree::bcf`, for causal inference with Bayesian additive regression trees (BART) [@hill2011]. For more on BART see @Chipman2010 and @sparapani2016. The package can be found on [CRAN](https://cran.r-project.org/package=bartCause).

We are following the procedure in @Hahn2020 (though there an automated version is possible with `stochtree::bcf`) where we estimate a propensity score for being assigned to the treatment regime, which improves estimation properties. The procedure is roughly as follows:
...
dat_covariates <- dat |> select(-y,-z, -subject_id)

vs_bart <- stochtree::bart(X_train = dat_covariates, y_train = dat$y, num_burnin = 5000, num_mcmc = 1000, 
                rfx_group_ids_train = dat$subject_id, 
                random_effects_params = list(model_spec = 'intercept_only')
                )

# subject_id as random effect!
...
covar_ranking <- covariate_importance(vs_bart, X_train = dat_covariates)

var_select <- covar_ranking |> 
  filter(avg_inclusion > max(avg_inclusion) - 2*sd(avg_inclusion)) %>% # at minimum: within 2 sd of max inclusion
  pull(variable)

var_select

# STEP 3 PS Model: Regress z ~ selected covariates
ps_bart <- bart(general_params = list(outcome_model = OutcomeModel(outcome = "binary")),
                X_train = dat_covariates, y_train = as.logical(dat$z), num_burnin = 5000, 
                num_mcmc = 1000, 
                rfx_group_ids_train = dat$subject_id, 
                random_effects_params = list(model_spec = 'intercept_only')
                )
...
prop_score <- epred_draws(ps_bart) |> 
  summarise(prop = mean(.value))

# Step 4 TE Model: Regress y ~ z + covariates + propensity score
te_bart <- bcf(X_train = dat_covariates, y_train = dat$y, 
               Z_train = dat$z,
               propensity_train = pull(prop_score, prop),
               rfx_group_ids_train = dat$subject_id, 
               num_burnin = 5000, num_mcmc = 1000, 
               random_effects_params = list(model_spec = 'intercept_only'))
