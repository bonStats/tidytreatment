...
suppressPackageStartupMessages({
  ...
  sim <- suhillsim_ranef1
  te_bart <- ""
  
  # pre compute
  dt = as.data.frame(sim$data)
  psbartmodel <- bartc(y, z, 
                       confounders = c1 + x1 + x2 + x3 + x4 + x5 + x6 + x7 + x8 + x9 + x10,
                       parametric = (1 | group_id), data = dt, 
                       commonSup.rule = "chisq",
                       commonSup.cut = 0.05)
...
We are following the procedure in @Hahn2020 (albeit without their more sophisticated model) where we estimate a propensity score for being assigned to the treatment regime, which improves estimation properties. This is done automatically in `bartCause`. The procedure is roughly as follows:
...
var_select
# includes all covariates, but could remove some below:
...
posterior_fitted <- fitted_draws(te_bart, value = "fit", include_newdata = FALSE)
...
# value came from (if we don't include the data in the result).
...
posterior_pred <- predicted_draws(te_bart, include_newdata = FALSE)
