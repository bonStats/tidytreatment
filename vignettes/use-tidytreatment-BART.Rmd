...
posterior_fitted <- fitted_draws(te_model, value = "fit", include_newdata = F)
...
posterior_pred <- predicted_draws(te_model, include_newdata = F)
...
```{r ate-trace-setup, eval = T, echo = F}
...
```{r common-support, echo=T, results='hide', cache=F}
...
We can count how many times a variable was included in the BART (on average) in conjunction with the treatment effect, or overall. This is a simple average see @bleich2014variable for more sophisticated methods.
...
res <- residual_draws(te_model, response = pull(dat, y), include_newdata = F)
res %>%   
